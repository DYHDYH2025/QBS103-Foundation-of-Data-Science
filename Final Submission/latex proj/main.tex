...
\begin{itemize}
    \item \texttt{tidyverse} \cite{tidyverse} for data manipulation and visualization
    \item \texttt{janitor} \cite{janitor} for variable name and table cleanup
    \item \texttt{gtsummary} \cite{gtsummary} for summary statistics tables
    \item \texttt{kableExtra} \cite{kableExtra} for LaTeX/HTML table formatting
    \item \texttt{pheatmap} \cite{pheatmap} for heatmap visualization
    \item \texttt{RColorBrewer} \cite{RColorBrewer} for colorblind-friendly palettes
    \item \texttt{ggplot2} \cite{ggplot2} for layered grammar of graphics
\end{itemize}
